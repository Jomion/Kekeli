...
async function chargerListe() {
  ...
  let requete = supabaseClient.from('quiz').select('*').order('created_at', { ascending: false });
...
function activerModeEdition(id, liste) {
  ...
  document.getElementById('classe').value = quiz.classe_id;
  remplirMatieres();
  document.getElementById('matiere').value = quiz.matiere_id || '';
  remplirSousMatieres();
  document.getElementById('sousMatiere').value = quiz.sous_matiere_id || '';
  ...
  document.getElementById('statut').value = quiz.statut;
...
document.getElementById('formAjout').addEventListener('submit', async (e) => {
  ...
  let resultat;
  if (quizEnEdition) {
    resultat = await supabaseClient.from('quiz').update(payload).eq('id', quizEnEdition);
  } else {
    resultat = await supabaseClient.from('quiz').insert(payload);
  }
